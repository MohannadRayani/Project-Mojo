...
public class Tower_Cannon : Tower
{
    ...
    {
        base.Awake();
        attackStrategy = new CannonAttackStrategy();
    }

    private sealed class CannonAttackStrategy : ITowerAttackStrategy
    {
        public void Execute(Tower tower)
        {
            Tower_Cannon cannon = (Tower_Cannon)tower;
            cannon.lastTimeAttacked = Time.time;

            Vector3 velocity = cannon.CalculateLaunchVelocity();
            cannon.attackVFX.Play();

            GameObject newProjectile = cannon.objectPool.Get(cannon.projectilePrefab, cannon.gunPoint.position, Quaternion.identity);
            newProjectile.GetComponent<Projectile_Cannon>().SetupProjectile(velocity, cannon.damage, cannon.objectPool);
        }
    }
